...
import org.leadingsoft.golf.api.entity.ApplyInfoTmp;
...
public class CollectService {
  ...
  public DataResult<CollectSearchResult> search(String playDate, String area, String golfBarName,
   ...
   if (StringUtils.isNotEmpty(roundSerialNo)) {
    	list = recruitInfoRepository.findRecruitInfoAtRoundSerialNoIsNotEmpty(memberId,ApovStatusCode.ApovOK.code(),playDate,Integer.valueOf(roundSerialNo),CollectStateCode.DELETE.code());
    }
    if (StringUtils.isNotEmpty(memberId)) {
    	list = recruitInfoRepository.findRecruitInfoAtMemberIdIsNotEmpty(memberId,ApovStatusCode.ApovOK.code(),playDate,memberId,CollectStateCode.DELETE.code());
    }*/
    
  //TODO entity: Applyinfo Model : ApplyInfo error
    ...
    Specification<RecruitInfo> specification = new Specification<RecruitInfo>(){
		...
		public Predicate toPredicate(Root<RecruitInfo> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
			...
			Root<CourseInfo> courseInfoRoot = query.from(CourseInfo.class);
			
			Join<RecruitInfo,CourseInfo> courseInfoJoin = root.join("courseInfos",JoinType.INNER);//param String join 的属性名
			
			Join<RecruitInfo,Applyinfo> applyinfoJoin = root.join("applyinfo",JoinType.LEFT); 
			applyinfoJoin.on(criteriaBuilder.equal(applyinfoRoot.get("memberId"),memberId));//相当于 join xxx on xx.xx = xx.xx and applyinfo.memberid = memberId;
			
			Join<RecruitInfo,ApplyInfoTmp> applyInfoTmpJoin = root.join("applyInfoTmp",JoinType.LEFT); 
